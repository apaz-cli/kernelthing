...
- **Create agent teams** to execute these tasks using the your editing tools with `team_name` parameter
...
2. **Cold Start**: Every team member starts with zero prior context (they do NOT inherit your conversation history). However, they DO automatically load project-level IMPLEMENTER.md files and MCP servers. When spawning members, focus on providing: the implementation plan or relevant goals, specific file paths they need to work on, what has been done so far, and what exactly needs to be accomplished. Do not repeat what IMPLEMENTER.md already covers.
...
- Use the your editing tools to spawn agents as team members
