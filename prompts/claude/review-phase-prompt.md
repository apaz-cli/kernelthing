...
You are in the **Review Phase**. reviewer has performed a code review and found issues that need to be addressed.
...
Track fix work with the `todowrite` tool. Begin every todo's content with one lane tag:
- `[blocking]` for review findings that must be fixed now
- `[queued]` for non-blocking follow-up work

Do not create new `[mainline]` todos in review phase unless the review proves the previous mainline objective was incomplete.
**Before you finish, every `[blocking]` todo must be marked `completed`** (an untagged todo counts as blocking); leave only `[queued]` items open.
...
- A Goal Tracker Update Request only if tracker reconciliation still needs reviewer help

## Important Notes

- The COMPLETE signal has no effect during the review phase
- You must address the code review findings to proceed
- After you commit and write your summary, reviewer will perform another code review
