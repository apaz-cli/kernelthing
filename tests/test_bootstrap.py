...
from kernelthing.config import Config, MARKER_COMPLETE, MARKER_SETUP_BLOCKED
...
def test_unique_dir_suffixes(tmp_path):
    (tmp_path / "gemm").mkdir()
    assert bootstrap._unique_dir(tmp_path, "gemm").name == "gemm-2"
...
def _author_complete_problem(target, *, edit_files='["kernel.py"]'):
    ...
    """
    (target / "problem.json").write_text(textwrap.dedent(f"""
    ...
    (target / "task.py").write_text("def generate_test_case(*, seed):\n    return (1,), (1, 0, 0)\n")
...
def _fake_run(text, *, author=True, **author_kw):
    ...
    def run(prompt, *, working_dir, **kw):
        ...
        return OpencodeResult(text=text, session_id="sess1", cost=0.0, tokens={},
                              exit_code=0)
    return run
...
def test_auto_happy_path_authors_and_commits(managed, monkeypatch, no_runtime_validation):
    monkeypatch.setattr(bootstrap.opencode_client, "run",
                        _fake_run(f"done\n{MARKER_COMPLETE}"))
    target = bootstrap.bootstrap_problem("fused softmax", cfg=Config(sandbox=False),
                                         auto=True, managed_root=managed)
    ...
    listed = subprocess.run(["git", "-C", str(target), "ls-files"],
...
def test_edit_files_may_not_include_adapter(managed, monkeypatch, no_runtime_validation):
    ...
    with pytest.raises(RuntimeError, match="adapter/spec files"):
        bootstrap.bootstrap_problem("x", cfg=Config(sandbox=False), auto=True,
...
def test_setup_blocked_raises(managed, monkeypatch, no_runtime_validation):
    ...
    with pytest.raises(RuntimeError, match="SETUP_BLOCKED"):
        bootstrap.bootstrap_problem("vague", cfg=Config(sandbox=False), auto=True,
                                    managed_root=managed)
...
def test_auto_validation_failure_raises(managed, monkeypatch, no_runtime_validation):
    ...
    monkeypatch.setattr(bootstrap.opencode_client, "run",
                        _fake_run(f"{MARKER_COMPLETE}", author=False))
