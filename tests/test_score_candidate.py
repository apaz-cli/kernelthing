...
def _orch(tmp_path):
    prob = Problem(name="p", repo_root=tmp_path, rel_dir=".", plan="plan.md",
                   edit_files=["k.cu"], score_command="echo")
...
def test_cheat_disqualified_without_scoring(tmp_path, monkeypatch):
    ...
    scored = []
    monkeypatch.setattr(orch, "_score_worktree",
                        lambda wt: scored.append(wt) or (True, 99.0, None))
    correct, metric, err = orch._guarded_score(tmp_path)
    assert correct is False
    assert metric is None
    assert err.startswith("kernelguard")
    assert scored == []           # the benchmark was never run for a cheat


def test_clean_candidate_is_scored(tmp_path, monkeypatch):
    orch = _orch(tmp_path)
    monkeypatch.setattr(gates, "kernelguard_violations", lambda *a, **k: [])
    monkeypatch.setattr(orch, "_score_worktree", lambda wt: (True, 99.0, None))
    correct, metric, err = orch._guarded_score(tmp_path)
